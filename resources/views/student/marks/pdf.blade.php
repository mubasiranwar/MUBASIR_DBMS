...
<head>
    ...
    <style>
        ...
        .section-header { background: #007bff; color: white; padding: 8px 12px; font-weight: bold; margin-bottom: 0; }
        .card { border: 1px solid #ddd; margin-bottom: 16px; }
        .card-body { padding: 12px; }
        table { width: 100%; border-collapse: collapse; }
        th, td { border: 1px solid #ddd; padding: 8px 10px; }
        th { background: #f8f9fa; font-weight: 600; width: 160px; }
        .badge { display: inline-block; padding: 3px 8px; border-radius: 3px; color: white; font-size: 11px; }
...
<body>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <table>
                <tr><th>Exam Name</th><td>{{ $mark->examTimetable->examType->name ?? 'N/A' }}</td></tr>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <table>
                ...
                <tr><th>Class</th><td>{{ $mark->examTimetable->schoolClass->class_name ?? 'N/A' }}</td><th>Section</th><td>{{ $mark->examTimetable->section->section_name ?? 'N/A' }}</td></tr>
                <tr><th>Subject</th><td colspan="3">{{ $mark->subject->subject_name ?? 'N/A' }}</td></tr>
            </table>
        </div>
    </div>
